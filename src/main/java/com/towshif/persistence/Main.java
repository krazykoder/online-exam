package com.towshif.persistence;
...
import com.towshif.persistence.model.Category;
		...
		try (AbstractApplicationContext context = new ClassPathXmlApplicationContext("applicationContext.xml")) {
			...
			for (Product product : Product.findAll()) {
				...
				String log = String.format(
						"%n-----------------------------------------------------" + "%n%-12s: %s" + "%n%-12s: %s"
								+ "%n%-12s: %s" + "%n%-12s: %s" + "%n%-12s: %s" + "%n%-12s: %.2f"
								+ "%n-----------------------------------------------------",
						"ID", product.getId(), "Category", categoryName, "Tags", tagName, "Product", product.getName(),
			...
			for (Order order : Order.findAll()) {
				String log = String.format("%n-----------------------------------------------------" + "%n%-18s: %s"
						+ "%n%-18s: %s" + "%n", "Order No", order.getId(), "Puchase orderdate",
						order.getPurchaseOrderdate());

				for (OrderLine orderLine : order.getOrderLines()) {
					String logline = String.format("%n  %d: %-36s %d x %.2f", orderLine.getPk().getOrderLineId(),
							orderLine.getProduct().getName(), orderLine.getQuantity(), orderLine.getAmount());
					log += logline;
				}
				log += String.format("%n" + "%n%-18s: %s" + "%n-----------------------------------------------------",
